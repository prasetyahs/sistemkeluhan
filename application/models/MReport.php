...
class MReport extends CI_Model
{
    ...
    {
        $query = $this->db->select("id_jnskeluhan")->get("tb_jsnkeluhan");
        return $query->result_array();
    }
    ...
    public function getKeluhan($col, $where)
    {
        ...
        $query  = $query = $this->db->from("tb_keluhan")->join("tb_pengguna", "tb_pengguna.id_pengguna = tb_keluhan.id_pelanggan")->join("tb_jsnkeluhan", "tb_jsnkeluhan.id_jnskeluhan = tb_keluhan.id_jnskeluhan")->order_by('nilai_alternatif', "DESC")->get();;
        return $query->result_array();
    }
    public function getPengecekan()
    {
        $this->db->group_by('tb_pengecekan.id_keluhan');
        $query = $this->db->from("tb_pengecekan")->join("tb_keluhan", "tb_keluhan.id_keluhan=tb_pengecekan.id_keluhan")->join('tb_pengguna', 'tb_pengguna.id_pengguna=tb_keluhan.id_pelanggan')->join('tb_jsnkeluhan', 'tb_jsnkeluhan.id_jnskeluhan=tb_keluhan.id_jnskeluhan')->get();
        return $query->result_array();
    }

    public function getCheckingReport($limit, $start)
    {
        $this->db->group_by('tb_pengecekan.id_keluhan');
        $query = $this->db->from("tb_pengecekan")->join("tb_keluhan", "tb_keluhan.id_keluhan=tb_pengecekan.id_keluhan")->join('tb_pengguna', 'tb_pengguna.id_pengguna=tb_keluhan.id_pelanggan')->join('tb_jsnkeluhan', 'tb_jsnkeluhan.id_jnskeluhan=tb_keluhan.id_jnskeluhan')->limit($limit, $start)->get();
        return $query->result_array();
    }
